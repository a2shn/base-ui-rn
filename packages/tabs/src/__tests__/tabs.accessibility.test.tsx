...
describe('Tabs - Accessibility', () => {
  it('has correct ARIA roles and attributes', () => {
    ...
    expect(tab.props['aria-selected'] ?? tab.props.accessibilityState?.selected).toBe(true);
  ...
  it('passes generic web accessibility props', () => {
    const { getByRole } = render(
      <Tabs.Root defaultValue="tab-1">
        <Tabs.List>
          <Tabs.Tab value="tab-1" aria-label="Home Tab"><Text>Home</Text></Tabs.Tab>
        </Tabs.List>
      </Tabs.Root>
    );

    const tab = getByRole('tab');
    expect(tab.props['aria-label'] ?? tab.props.accessibilityLabel).toBe('Home Tab');
  });
});
