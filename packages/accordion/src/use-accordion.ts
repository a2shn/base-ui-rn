...
import type { LayoutChangeEvent, NativeSyntheticEvent, View } from 'react-native';
...
export function useAccordionItem(props: AccordionItemProps) {
  ...
  // 1. Create the stable ref here
  const triggerRef = React.useRef<View | null>(null);

  React.useLayoutEffect(() => {
    // 2. Register the stable ref directly
...
export function useAccordionTrigger(props: AccordionTriggerProps) {
  ...
  const isDisabled = disabledProp === true || itemContext.isDisabled || rootContext.isDisabled;
