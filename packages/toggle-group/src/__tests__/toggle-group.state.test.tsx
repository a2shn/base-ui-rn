...
describe('ToggleGroup - State Management', () => {
  ...
  describe('Uncontrolled Value', () => {
    ...
    it('defaults to an empty array when no defaultValue is provided', () => {
      ...
      expect(getByRole('checkbox', { name: 'A' }).props.accessibilityState.checked).toBe(false);
    });
  });
  ...
  describe('Controlled Value', () => {
    it('follows the controlled value prop', () => {
      ...
      expect(getByRole('checkbox', { name: 'A' }).props.accessibilityState.checked).toBe(true);
      expect(getByRole('checkbox', { name: 'B' }).props.accessibilityState.checked).toBe(false);
      ...
      expect(getByRole('checkbox', { name: 'A' }).props.accessibilityState.checked).toBe(false);
      expect(getByRole('checkbox', { name: 'B' }).props.accessibilityState.checked).toBe(true);
    });
    ...
    it('calls onValueChange when a toggle is pressed', () => {
      ...
      expect(onValueChange).toHaveBeenCalledWith(['b'], expect.objectContaining({
        source: 'press',
        value: 'b'
      }));
    });
